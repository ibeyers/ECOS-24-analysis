...
model CavernECOS_flat_IdealGas
  ...
  parameter Volume V_cavern = 2490000 "Volume of cavern";
  parameter Area A_eff = 249000 "Effective surface area of cavern";
  parameter Pressure p_max_op = from_bar(120) "Chosen maximum operating pressure level";
  parameter Pressure p_min_op = from_bar(50) "Chosen minimum operating pressure level";
  // Initial values
  parameter Pressure p_cavern_initial = from_bar(75) "Initial cavern pressure";
  parameter Temperature T_cavern_initial = from_degC(39) "initial cavern temperature";
  parameter Temperature T_cavern_infinite = from_degC(39) "infinite temperature of surrounding salt-rock";
  ...
  parameter Temperature T_in = from_degC(60) "Temperature of the hydrogen gas that is injected into the cavern";
...
//energy balance
  m_H2_cavern*Medium.specificHeatCapacityCv(HydrogenThermState)*der(T_H2_cavern) = Q_dot_rock + m_dot_H2_in*HydrogenStateInput.h - m_dot_H2_out*HydrogenState.h - (HydrogenState.h - p_cavern*1/HydrogenState.d)*der(m_H2_cavern);
